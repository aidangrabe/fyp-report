...
\item Eclipse\\
    ...
    (Android Development Tools) to interact with ADB and manage SDKs etc.
\item Android Studio\\
...
respond to received messages, the device must register itself with the Android Wear
...
updated on one side, an event is triggered on the other notifying it of the
change.
...
carried out asynchronously with callback methods called in background threads.
...
immediately after as the user's hand comes to a stop. In order to ignore this
...
need to be launched by the user. They are a way of displaying short glanceable
...
is all about and are easily created and displayed using the Android APIs.
...
\item News\\
    ...
    of a native Android RecyclerView, the articles are shown when the article
    image or headline is clicked by the user.
\item Campus Map\\
...
and works quite quickly as the module objects are small in size and quick to
transfer. It is not the most optimal method for syncing the modules. A better
approach would be to implement a database on the wearable device as well as the
...
would be a far better approach and would provide a noticeably better user
experience, in terms of a more responsive UI.
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{mobile-timetable.png}
    \caption{Timetable screen}
    \label{fig:mobile_timetable}
\end{figure}

The timetable screen displays the user's lectures for the week in the form of
tabbed lists (Figure \ref{fig:mobile_timetable}).
Each day is its own tab and displays the days lectures in order
...
is very simple. In order for a Java object to be stored in the database using
...
could be used on both the handheld and wearable devices. The main reason for
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{mobile-results.png}
...
parsing it using the \texttt{Jsoup}\footnote{Jsoup library: http://jsoup.org}
library to find the news article titles and
...
\texttt{View Holder} pattern, making it more efficient by recycling (hence the
...
library. This library makes downloading things very easy, without having to worry about
closing connections, or configuration changes causing the Activity to restart.
Since Android does not allow network activity on the main thread, network
...
\texttt{MyVolley} is just a simple wrapper for a Volley \texttt{RequestQueue}.
...
user on both the handheld and wearable devices. The wearable notifications
...
row number would change by 1. However with a \texttt{GridViewPager} the column
...
the corners are truncated off screen and so cannot be clicked.
...
of being able to handle input/clicks for each individual button in the grid,
...
not finalized nor made publicly available, so an attempt was made to use
...
to be used, which is a more powerful graphics API than canvas, but also has a
...
canvas to ensure that it is visible on all screen shapes. A custom listener is
shown below:
...
events along the circumference of the watchface as long coloured arcs
(Figure \ref{fig:calendar_watchface}). However
...
\begin{figure}
    ...
    \caption{The custom Watchface with the events as coloured arcs}
...
\subsection{Calendar WatchFace}

An extension of the watchface developed for this project was published to the
Google Play
Store\footnote{Calendar WatchFace: https://play.google.com/store/apps/details?id=com.aidangrabe.customwatchface}.
This app is built on the same source code as the watchface for this project, but
also adds many features such as a settings application for mobile that allows
the users to customize different settings for the watchface such as the
thickness of the hands and the background image for the clock face. At the time
of writing the watchface has been purchased by 18 people and has a rating of 4.3
in the Play Store.
